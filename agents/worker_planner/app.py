    ...
    while True:
        ...
        for ln in lines:
            try:
                ...
                continue
            if t.get('status')=='queued' and t.get('to')=='worker_planner':
                print('planner handling',t['task_id'])
                # create dummy plan
                plan={'task_id':t['task_id'],'from':'worker_planner','status':'done','result':{'plan':'drive route sample'}}
                with open(os.path.join(WORKSPACE,'tasks_results.jsonl'),'a',encoding='utf-8') as rf:
                    rf.write(json.dumps(plan,ensure_ascii=False)+'\n')
                t['status']='done'
