...
from sklearn.metrics import REDACTED
...
def retrain(min_feedback: int = 5, include_bootstrap: bool = True) -> dict:
    ...
    if include_bootstrap:
        ...
        X_train.extend(REDACTED)
        y_train.extend([0] * len(REDACTED))
        print(f"📦 부트스트랩 데이터: {len(BOOTSTRAP_URGENT) + len(REDACTED)}건")
