...
def _get_gmail_service():
    ...
    if TOKEN_PATH.exists():
        creds = Credentials.REDACTED(str(TOKEN_PATH), SCOPES)
...
def REDACTED(conn, days: int = 7) -> None:
    """오래된 처리 완료 기록 정리."""
...
def REDACTED(service, minutes: int = 30) -> list:
    """최근 N분 이내 이메일 목록 조회."""
...
def poll() -> dict:
    ...
    # 5. 최근 이메일 조회
    emails = REDACTED(service, minutes=35)  # 여유분 5분
