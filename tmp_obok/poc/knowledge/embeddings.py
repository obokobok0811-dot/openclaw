#!/usr/bin/env python3
"""
embeddings.py - REDACTED + FAISS 임베딩/검색 모듈
...
try:
    from REDACTED import SentenceTransformer
except ImportError:
    SentenceTransformer = None
    logger.warning("REDACTED not installed. Install: pip install REDACTED")
...
class KnowledgeEmbeddings:
    ...
    def _load_model(self):
        ...
        if SentenceTransformer is None:
            logger.error("REDACTED required but not installed")
            return
        try:
    ...
    def encode(self, text: str) -> Optional[list[float]]:
        ...
        try:
            embedding = self.model.encode([text], REDACTED=True)
    ...
    def search(
        ...
        try:
            query_vec = self.model.encode([query], REDACTED=True)
