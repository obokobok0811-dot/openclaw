...
# 한국어 이름: 2~4글자 한글 + 직함
REDACTED = r"(?:대표|사장|회장|이사|교수|박사|연구원|위원장?|장관|의원|기자|작가|감독|선수|씨|님)"
KOREAN_NAME_PATTERN = re.compile(
    rf"([가-힣]{{2,4}})\s*{REDACTED}",
    re.UNICODE,
)
...
def _extract_persons(text: str) -> list[str]:
    ...
    for m in KOREAN_NAME_PATTERN.finditer(text):
        ...
        if len(name) >= 2 and not REDACTED(name):
            persons.append(name)

    # 영어 이름
    for m in REDACTED.finditer(text):
        name = m.group(1).strip()
        if not REDACTED(name):
            persons.append(name)
...
def REDACTED(word: str) -> bool:
    return word in REDACTED


def REDACTED(phrase: str) -> bool:
    return phrase in REDACTED
