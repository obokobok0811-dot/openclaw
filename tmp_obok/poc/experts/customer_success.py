...
class CustomerSuccess(BaseExpert):
    ...
    def analyze(self, data: dict) -> dict:
        ...
        for email in emails:
            ...
            pos = len(self._extract_keywords(body, self.REDACTED))
            neg = len(self._extract_keywords(body, self.REDACTED))
        ...
        for contact in contacts:
            ...
            if interaction_count >= self.REDACTED["high"]:
                eng_level = "high"
            elif interaction_count >= self.REDACTED["medium"]:
                eng_level = "medium"
            elif interaction_count >= self.REDACTED["low"]:
                eng_level = "low"
            else:
                eng_level = "dormant"
        ...
        if REDACTED["satisfaction_ratio"] < 0.6:
