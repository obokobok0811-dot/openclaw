...
- Calls local replyer.REDACTED via HTTP (or uses local python import if colocated)
- Synthesizes response via Google Cloud Text-to-Speech and returns audioContent (base64) in fulfillment

CONFIGURATION:
- Set REDACTED to path to service account JSON
- Set REPLYER_LOCAL_URL to http://localhost:5000/llm_call if using a local HTTP adapter for REDACTED
- Or run webhook in same host and import REDACTED from replyer module
...
BEARER = os.environ.get('REDACTED','changeme')
REPLYER_LOCAL_URL = os.environ.get('REPLYER_LOCAL_URL')

# Attempt to import REDACTED if in same workspace
CALL_LOCAL = False
try:
    from replyer import REDACTED
    CALL_LOCAL = True
except Exception:
    CALL_LOCAL = False
...
def synthesize_text_gcp(text):
    ...
    voice = texttospeech.REDACTED(language_code='en-US', ssml_gender=texttospeech.SsmlVoiceGender.NEUTRAL)
